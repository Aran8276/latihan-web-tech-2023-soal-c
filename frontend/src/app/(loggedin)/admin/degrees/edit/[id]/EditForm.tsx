...
import { DegreesRequest, editDegree } from "../../actions";
...
import { Degrees } from "@/app/fetcher";
import { toast } from "sonner";

export default function EditForm({ formFields }: { formFields: Degrees }) {
  ...
  const editFunction = async (payload: DegreesRequest) => {
    if (payload.degree_name && payload.is_enabled) {
      setState({ loading: true, error: "" });
      const processCreate = await editDegree(payload, formFields.id);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-3">
      ...
      <div className="flex flex-col space-y-3">
        ...
        <Input
          defaultValue={formFields.degree_name}
          id="degree_name"
          name="degree_name"
          type="text"
        />
